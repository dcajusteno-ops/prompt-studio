...
- 📦 **强大的批量操作**：支持按住 Shift/Ctrl 进行多选，或者一键全选，实现批量删除、批量移动画集、批量设置限制级等。
- 🛡️ **智能限制级分级防护 (v0.2+)**：为图片设置独立分级（Safe, R, X, XX, XXX）。全局一键开启/关闭“保护模式”，敏感内容自动毛玻璃遮挡，支持临时点击查看。
- ⚙️ **灵活的系统设置 (v0.2+)**：支持设置默认图片上传分级，保护隐私更省心。
- 💾 **一键归档与恢复 (v0.2+)**：系统设置中提供完整的 Zip 归档导出和导入覆盖功能，跨设备迁移或定期备份随心所欲。
- 🚀 **极简的本地存储**：所有数据及图片均安全地保存在本地 `database/` 和 `public/uploads/` 目录下。

## 🚀 快速启动

为方便日常使用，项目根目录提供了一个一键启动脚本：
- **Windows**：双击运行根目录下的 `启动服务.bat` 即可。**初次运行该脚本时会自动安装所需的依赖环境**，随后会自动启动服务并在浏览器中打开 `http://localhost:33333`。
...
### v0.2.2
- 🐛 **Bug Fix**: 修复了在应用内删除图片时，硬盘上的原始图片文件没有被同步删除的问题。
